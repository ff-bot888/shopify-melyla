 ...
 * @property {HTMLElement} mediaList - The media list element.
 * @property {HTMLElement} thumbnails - The thumbnails elements.
 *
 * @extends Component<Refs>
 */
export class ZoomDialog extends Component {
  requiredRefs = ['dialog', 'media', 'mediaList', 'thumbnails'];

  #highResImagesLoaded = /** @type {Set<string>} */ (new Set());

  connectedCallback() {
    super.connectedCallback();
    this.refs.mediaList.addEventListener('scroll', this.handleScroll);
  }

  disconnectedCallback() {
    super.disconnectedCallback();
    this.refs.mediaList.removeEventListener('scroll', this.handleScroll);
  }
  ...
  handleScroll = debounce(async () => {
    const { media, thumbnails, mediaList } = this.refs;

    const mostVisibleElement = await getMostVisibleElement(media, mediaList);
  ...
  async close() {
    ...
    const mostVisibleElement = await getMostVisibleElement(media, this.refs.mediaList);
  ...
  /**
   * Toggle desktop image zoom for the active media.
   * @param {number} index - The index of the image media item.
   * @param {MouseEvent} [event] - The click event.
   */
  toggleImageZoom(index, event) {
    if (window.matchMedia('(max-width: 749px)').matches) return;

    const targetMedia = this.refs.media[index];
    if (!targetMedia) return;

    const wrapper = targetMedia.querySelector('drag-zoom-wrapper');
    ...
    if (event) {
      const image = wrapper.querySelector('img.product-media__image');
      const targetElement = image instanceof HTMLElement ? image : wrapper;
    ...
    wrapper.style.setProperty('--desktop-zoom-origin', zoomOrigin);
    wrapper.style.setProperty('--desktop-zoom-scale', '2.15');
  }

  resetDesktopZoom() {
    const wrappers = this.querySelectorAll('drag-zoom-wrapper.is-zoomed');
    wrappers.forEach((wrapper) => {
      if (!(wrapper instanceof HTMLElement)) return;
      wrapper.classList.remove('is-zoomed');
      wrapper.style.removeProperty('--desktop-zoom-scale');
      wrapper.style.removeProperty('--desktop-zoom-origin');
    });
  }
}
 ...
 */
function getMostVisibleElement(elements, root = null) {
